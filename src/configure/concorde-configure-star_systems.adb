...
with Accord.Atmosphere;
with Accord.Atmospheric_Gas;
--  with Accord.Palette;
with Accord.Star;
with Accord.World;
...
package body Concorde.Configure.Star_Systems is
   ...
   use all type Accord.Db.World_Composition;
   subtype World_Composition is Accord.Db.World_Composition;

   subtype Rocky_World is World_Composition range Ice .. Rock_Iron;

   use all type Accord.Db.Stellar_Orbit_Zone;
   subtype Orbit_Zone is Accord.Db.Stellar_Orbit_Zone;
...
--        Jungle, Forest);
   ...
   is (Concorde.Random.Unit_Random * 5.0 + 1.0);
   ...
   package Planet_Tables is
      ...
      function Composition
        (Mass : Non_Negative_Real;
         Zone : Orbit_Zone)
         return Accord.Db.World_Composition;

      function Random_Planet_Density
        (Composition : Accord.Db.World_Composition;
         Mass        : Non_Negative_Real)
         return Non_Negative_Real;
   ...
   package body Planet_Tables is
      ...
      type Zone_Composition is
        array (Planetary_Zone) of Accord.Db.World_Composition;
      ...
      is
         ...
         Step_Roll  : constant Unit_Real := Concorde.Random.Unit_Random;
      ...
      begin
         return Concorde.Random.Unit_Random * (High - Low) + Low;
      ...
      is
         ...
         Base : constant Non_Negative_Real :=
                  Real (D (N)) * (0.8 + Concorde.Random.Unit_Random * 0.4);
      begin
         if Mass < 10.0 then
            if Orbit < 0.3 then
               return Year * Concorde.Solar_System.Earth_Sidereal_Year;
   ...
   is
      Star : constant Accord.Star.Star_Handle :=
               Accord.Star.First_By_Star_System (Star_System);
   ...
   begin
      ...
      if Log_Generation then
         Put (16, Star.Name);
         Ada.Text_IO.Put (Accord.Db.Spectral_Class'Image (Star.Class));
   ...
   is
      ...
      Elevation_Range : constant Natural :=
        Natural
          ((Concorde.Random.Normal_Random (0.1) + 2.0) * 25.0);
      ...
      use all type Accord.Db.Life_Complexity_Type;

      Life_Complexity : constant Accord.Db.Life_Complexity_Type :=
                          (if not Life_Bearing
                           then No_Life
      ...
      Climate             : Accord.Db.World_Climate;
   ...
   begin
      ...
      Climate :=
        (if Gas_Giant
         then Accord.Db.Jovian
         elsif Current_Pressure = 0.0
         then Accord.Db.Airless
         elsif Terrain (Ice) > 0.9
         then Accord.Db.Iceball
         elsif Hydrosphere > 0.9
         then Accord.Db.Water
         elsif Hydrosphere < 0.1
         then Accord.Db.Desert
         elsif Atmospheric_Class = Dense
         then Accord.Db.Venusian
         elsif Atmospheric_Class = Thin
         then Accord.Db.Martian
         else Accord.Db.Temperate);

      declare
         use all type Accord.Db.World_Climate;
         ...
         World : constant Accord.World.World_Handle :=
           Accord.World.Create
             (Star_System         => Star.Star_System,
              ...
              Name                => Name,
--                Palette             =>
--                  Accord.Palette.Get_By_Tag ("temperate"),
   ...
   is
      Lum : constant Non_Negative_Real :=
              Concorde.Elementary_Functions.Sqrt (Star.Luminosity);
   begin
   ...
   is
   begin
      Put (Width, Concorde.Real_Images.Approximate_Image (Value));
   end Put;
...
end Concorde.Configure.Star_Systems;
