...
package body Concorde.Identifiers is
   ...
   private
      Current : Accord.Next_Identifier.Next_Identifier_Handle;
   ...
   protected body Identifier_Source is
      ...
      begin
         if not Current.Has_Element then
            Current :=
              Accord.Next_Identifier.First_By_Top_Record
                (Accord.Db.R_Next_Identifier);
         end if;

         if not Current.Has_Element then
            Current :=
              Accord.Next_Identifier.Create
                (Template);
         end if;
