...
package body Concorde.Calendar is
   ...
   Clock_Handle : Accord.Calendar.Calendar_Handle;
   ...
   begin
      if not Clock_Handle.Has_Element then
         Clock_Handle :=
           Accord.Calendar.First_By_Top_Record
             (Accord.Db.R_Calendar);
      end if;
   end Check_Clock;
   ...
   is
   begin
      Year := Concorde.Calendar.Year (Date);
      Month := Concorde.Calendar.Month (Date);
      Day := Concorde.Calendar.Day (Date);
      Seconds := Concorde.Calendar.Seconds (Date);
   end Split;
